...
%--------------------------------------------------------------------------
    ...
    properties(SetAccess = protected)
        ...
        pro;                            % Problem solved in current execution测试问题
        result;                         % Populations saved in current execution
        metric;                         % Metric values of current populations评估指标
    ...
    methods
        ...
        %   ALGORITHM, which is usually called by ALGORITHM.Solve.在每个特定算法中继承
        end
    end
    methods(Access = protected, Sealed)
        ...
        %       end
            ...
            obj.result(index,:) = {obj.pro.FE,Population};%存储每一次迭代的种群
            drawnow('limitrate');
            obj.outputFcn(obj,obj.pro);
            nofinish = obj.pro.FE < obj.pro.maxFE;%判断是否达到最大迭代次数
            % nofinish = (obj.pro.FE < obj.pro.maxFE) || (obj.pro.count_rowstodelete >= 1);
...
% The default output function of ALGORITHM
    ...
    if Problem.FE >= Problem.maxFE
        ...
        elseif Algorithm.save > 0
            ...
            assignin('base', 'result', result);%%%%%%新加
        end
    end
