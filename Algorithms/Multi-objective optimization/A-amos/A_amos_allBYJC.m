classdef A_amos_allBYJC < ALGORITHM
% <multi> <real/integer>
        ...
        function main(Algorithm, Problem)
           ...
           while Algorithm.NotTerminated(Population)
             ...
             for i = 1:popSize
                  ...
                  %% 消融对比1：每个体先交叉再变异（父代与主算法一致：交叉=当前+不同类随机一个，变异=作用在交叉结果上，等价于主算法变异用自身）
                  explorationRate = min(0.70, 0.25 + stagnationCounter * 0.02);
                  
                  % 与 A_amos 一致：交叉 parent1=当前个体，parent2=不同类中随机一个
                  parent1 = currentSol;
                  ...
                  if isequal(parent2, parent1) && popSize >= 2 && length(otherClustIdx) > 1
                      idx2 = otherClustIdx(randsample(length(otherClustIdx), 1));
                      parent2 = pop(idx2, :);
                  end
                  
                  % 先交叉，再变异（每个体均经历交叉+变异）
                  [trialSol, crossDbg] = SegmentBasedCrossover(parent1, parent2, n, m, varDim);
                  ...
                  [trialSol, mutDbg] = OrderPreservingMutation(trialSol, m, varDim, n, explorationRate);
