classdef A_amos_noADJUST < ALGORITHM
% <multi> <real/integer>
        ...
        function main(Algorithm, Problem)
           ...
           while Algorithm.NotTerminated(Population)
             ...
             for i = 1:popSize
                   ...
                   %% 父代逻辑（与 A_amos 一致）：交叉=当前+不同类，变异=类中随机；noADJUST 仅固定概率不随代调整
                   % [已注释] 原父代选择策略：localSearchProb、邻域/全局选 parent
                   
                 %% noADJUST：交叉与变异概率始终 0.5（不随代数调整）
                 crossoverProb = 0.5;
                 ...
                 if useCrossover && popSize >= 2
                     ...
                     % 变异：父代从当前类中随机选一个（含当前个体）
                     sameClustIdx = find(clustTag == currentTag);
                     if isempty(sameClustIdx)
                         parent = currentSol;
                     else
                         idxMut = sameClustIdx(randsample(length(sameClustIdx), 1));
                         parent = pop(idxMut, :);
                     end
                  ...
                  %     end
                  % end
                % 2. 定期（每10代）对所有个体增加一次额外变异，打破局部最优（noADJUST 中关闭该额外变异）
