function [pop,objvs,clustTag,clustName,centroid]=GeOACES(auxPop,auxObjvs,pop,objvs,clustTag,clustName,selectedSize,objDim,varDim,Kmax)
...
[rk,~]=NDSort(objvs,inf);                                                      % Rank the population
refPoint=max(objvs,[],1); refPoint=1.2*refPoint;                           % Construct the reference point
...
if i~=1
    ...
    FiObjvs=objvs(rk==i,1:objDim); FiPop=pop(rk==i,1:varDim); FiTag=clustTag(rk==i,1);
    if objDim==2
    ...
    pop=[auxPop;FiPop]; objvs=[auxVals;FiObjvs]; clustTag=[auxTag;FiTag];  % The selected population
end
