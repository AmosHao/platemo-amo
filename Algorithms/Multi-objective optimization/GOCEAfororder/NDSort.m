...
%--------------------------------------------------------------------------
    ...
    PopObj = varargin{1};%从输入参数列表 varargin 中获取第一个参数值，并赋给 PopObj 变量
    [N,M]  = size(PopObj);
    if nargin == 2
        nSort  = varargin{2};
    else
        PopCon = varargin{2};
        nSort  = varargin{3};
        Infeasible           = any(PopCon>0,2);%根据约束条件 PopCon 判断每个个体是否不可行（即是否存在约束违反），将结果存储在逻辑向量 Infeasible 中。
        PopObj(Infeasible,:) = repmat(max(PopObj,[],1),sum(Infeasible),1) + repmat(sum(max(0,PopCon(Infeasible,:)),2),1,M);%将所有不可行个体的目标值替换为一个特定值：最大目标值和不可行约束值的和。
    end
    if M < 3 || N < 500
...
function [FrontNo,MaxFNo] = ENS_SS(PopObj,nSort)
    [PopObj,~,Loc] = unique(PopObj,'rows');%对popobj每行去重，返回去重后的矩阵和每一行在唯一化后的数组中的位置
    Table   = hist(Loc,1:max(Loc));%统计去重后的 PopObj 矩阵中每个行向量出现的次数，存储在 Table 中。
    [N,M]   = size(PopObj);
    FrontNo = inf(1,N);%初始化 FrontNo 数组为大小为 1 x N 的数组，初始值为无穷大 inf，用于存储每个个体所在的 Pareto Front 号码。
    MaxFNo  = 0;
