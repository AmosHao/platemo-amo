classdef GOCEAforplanner< ALGORITHM
% <multi> <real/integer>
% GOCEA
% Kmax ---   11 --- The K
% BETA    --- 0.5 --- The B
        ...
        function main(Algorithm, Problem)
           ...
           while Algorithm.NotTerminated(Population)
               ...
               for i=1:popSize%遍历整个种群中的个体，为每个个体确定其邻居集合
                   ...
                   trialSol = updateArcCenters(trialSol);
                   trialVal=Problem.Evaluation(trialSol);
                   auxPop(i,:)=trialSol; auxVals(i,:)=trialVal;
               end
               auxObjvs=auxVals.objs;
               selectedSize=Problem.N;
               [auxall,pop,objVals,clustTag,clustName,centroid]=GeOACESforplanner(auxPop,auxObjvs,pop,objVals,clustTag,clustName,selectedSize,objDim,varDim,Kmax,auxVals,Population);
               Population=auxall;
               popSize=size(pop,1);
               for i = 1:popSize
