classdef GOCEAfortravel< ALGORITHM
% <multi> <real/integer>
% GOCEA
% Kmax ---   11 --- The K
% BETA    --- 0.5 --- The B
...
% CR    --- 1 --- The CR
    methods
        function main(Algorithm, Problem)
           %% parameter setting
           [Kmax,BETA,F,CR] = Algorithm.ParameterSet(11,0.5,0.5,1);
           popSize = Problem.N; 
           objDim=Problem.M;
           varDim=Problem.D;
           pm=1.0/varDim;
           ...
           while Algorithm.NotTerminated(Population)
               ...
               for i=1:popSize%遍历整个种群中的个体，为每个个体确定其邻居集合
                   ...
                   parents_a=parents(:,6:end);
                   trialSol=DifferentialEvolutionCrossover(parents_a,bounds,F,CR); trialSol=PolynomialMutation(trialSol,bounds,pm);
                   trialSol=[parents(1,1:5),trialSol];%选了母代排列的第一个
                   trialVal=Problem.Evaluation(trialSol);
                   auxPop(i,:)=trialSol; auxVals(i,:)=trialVal;
               end
               auxObjvs=auxVals.objs;
               selectedSize=Problem.N;
               [auxall,pop,objVals,clustTag,clustName,centroid]=GeOACESfortravel(auxPop,auxObjvs,pop,objVals,clustTag,clustName,selectedSize,objDim,varDim,Kmax,auxVals,Population,Problem);
               Population=auxall;
               popSize=size(pop,1);
               for i = 1:popSize
