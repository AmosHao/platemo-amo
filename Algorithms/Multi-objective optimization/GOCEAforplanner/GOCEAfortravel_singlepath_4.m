classdef GOCEAfortravel_singlepath_4< ALGORITHM
% <multi> <real/integer>
% GOCEA
% Kmax ---   15 --- The K
% BETA    --- 0.5 --- The B
...
% for i=1:size(pop,1)
% dec1=pop(i,1:n_dots);
% dec2=pop(i,n_dots+1:2*n_dots);
% dec3=pop(i,2*n_dots+1:3*n_dots);
...
% hold on;
               ...
               [auxa,auxb,auxc,auxall,pop,objVals,clustTag,clustName,centroid]=GeOACESfortravel_singlepath_4(auxPop,pop,clustTag,clustName,selectedSize,objDim,varDim,Kmax,Problem,buildings,numBuildings,cylinders,numCylinders,spheres,numSpheres,pyramids,numPyramids,jinfei,numjinfei,dot1,dot2);
               Population=auxall;
               popSize=size(pop,1);
...
% hold on;
               ...
               for i = 1:popSize
                   ...
                   % [pop(i,:),validPoints]=checknode_singlepath_close_new(pop(i,:),varDim,buildings,numBuildings,cylinders,numCylinders,spheres,numSpheres,pyramids,numPyramids,jinfei,numjinfei,dot1,dot2);%对当前解包含的航迹点进行碰撞检测         
                   % [validPoints]=checknode_singlepath_close(pop(i,:),varDim,buildings,numBuildings,cylinders,numCylinders,spheres,numSpheres,pyramids,numPyramids,jinfei,numjinfei);%对当前解包含的航迹点进行碰撞检测
                   % c=[c;validPoints'];
                   % if any(~validPoints) 
                   % d=[d;0];
...
% pop_cd=pop;
% objcheck=Population.objs;
% deccheck=Population.decs;
% validtraitcheck=[];
% for kk=1:size(deccheck,1)
% validtraitcheck(kk,:)=Population(kk).validtrait;
% end
