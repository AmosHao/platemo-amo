function [auxall,pop,objvs,clustTag,clustName,centroid]=GeOACESforplanner(auxPop,auxObjvs,pop,objvs,clustTag,clustName,selectedSize,objDim,varDim,Kmax,auxVals,Population)
...
%读取cubes数据
% 指定Excel文件名和工作表
filename = 'cubes_data_curve_small_11.xlsx';
sheet = 1;  % 假设数据在第一个工作表中
% 使用xlsread读取数据
cubesdata = xlsread(filename, sheet);
collision_indices = check_collisions(pop, cubesdata,varDim);
a=sum(collision_indices);
if a<5%保证至少5个个体
% 找出所有 false 的位置
false_indices = find(~collision_indices);
% 对 false_indices 进行排序（如果需要按升序或降序排序可以用 sort 函数）
sorted_false_indices = sort(false_indices);
% 选择前 num_to_find 个 false 的位置
loc = sorted_false_indices(1:size(pop,1)-8);
else
loc = find(~collision_indices);
end
objvs(loc,:)=[];
pop(loc,:)=[]; clustTag(loc,:)=[];allVals(loc)=[];
...
if i-1~=1
    ...
    if objDim==2
        ...
        %     FiObjvs(worst,:)=[]; FiPop(worst,:)=[]; FiTag(worst,:)=[];auxall(worst)=[];
        %     % FiSize=FiSize-1;
        %     % auxSize=auxSize-1;
        % end
