function [auxall,pop,objvs,clustTag,clustName,centroid]=GeOACESfortravel_singlepath(auxPop,auxObjvs,pop,objvs,clustTag,clustName,selectedSize,objDim,varDim,Kmax,auxVals,Population,Problem)
...
auxVals=[]; auxPop=[]; auxTag=[];auxSize=0; auxall=[];                               % Add solutions into the auxiliary population
...
tic;
        while auxSize-selectedSize>0
            ...
            [~,loc]=min(fitV);
            FiObjvs(loc(1,1),:)=[]; FiPop(loc(1,1),:)=[]; FiTag(loc(1,1),:)=[];auxall(loc(1,1))=[];
            FiSize=FiSize-1;
            auxSize=auxSize-1;
        end
    end
...
pop=pop(clustTag~=inf,1:varDim); objvs=objvs(clustTag~=inf,1:objDim);auxall=auxall(:,clustTag~=inf); clustTag=clustTag(clustTag~=inf,1);

for i=1:size(popa,1)
    y=popa(i,1:varDim); yObjv=objvsa(i,1:objDim);ypopulation=auxallpopa(:,i);
    [auxall,pop,objvs,clustTag,clustName,centroid]=OACES_(auxall,y,yObjv,ypopulation,pop,objvs,clustTag,clustName,centroid,varDim,Kmax);
end
end
...
function [auxall,pop,objvs,clustTag,clustName,centroid]=OACES_(auxall,y,yObjv,ypopulation,pop,objvs,clustTag,clustName,centroid,varDim,Kmax)
if isempty(clustName)%如果所有先前分好类的个体为空，即全部被删除
    pop=[pop;y]; objvs=[objvs;yObjv];auxall=[auxall,ypopulation]; clustTag=1; clustName=1; centroid=y;
else
