function [auxall,pop,objvs,clustTag,counter,centroid]=OACES_hv_a(y,yObjv,pop,objvs,clustTag,counter,centroid,popSize,objDim,varDim,Kmax,auxVals,Population)
...
[rk,~]=NDSort(objvs,inf); auxVals=auxVals';auxall=[Population,auxVals];                                                     % Rank the population                           % Construct the reference point
refPoint=max(objvs,[],1); refPoint=1.2*refPoint; 

if max(rk)~=1
        d=NumberOfDominatingPoints(objvs); [~,loc]=max(d);
        if loc~=popSize
        [auxall,pop,objvs,clustTag,counter,centroid]=OACES_(auxall,auxVals,y,yObjv,loc,pop,objvs,clustTag,counter,centroid,popSize,objDim,varDim,Kmax); % Delete the solutions with the worst rank and highest d value
        else
         pop(popSize,:)=[]; objvs(popSize,:)=[]; clustTag(popSize,:)=[];auxall(:,popSize)=[];
        end
else
    if objDim==2
        frontObjvs=objvs;
        ...
        [~,loc]=min(fitV);
        if loc(1,1)~=popSize
            [auxall,pop,objvs,clustTag,counter,centroid]=OACES_(auxall,auxVals,y,yObjv,loc,pop,objvs,clustTag,counter,centroid,popSize,objDim,varDim,Kmax);
        else
            pop(popSize,:)=[]; objvs(popSize,:)=[]; clustTag(popSize,:)=[];auxall(:,popSize)=[];
        end
    else
...
function [auxall,pop,objvs,clustTag,counter,centroid]=OACES_(auxall,auxVals,y,yObjv,loc,pop,objvs,clustTag,counter,centroid,popSize,objDim,varDim,Kmax)
