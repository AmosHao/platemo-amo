function [auxa,auxb,auxc,auxall,pop,objvs,clustTag,clustName,centroid]=GeOACESfortravel_singlepath_5(auxPop,pop,clustTag,clustName,selectedSize,objDim,varDim,Kmax,Problem,buildings,numBuildings,cylinders,numCylinders,spheres,numSpheres,pyramids,numPyramids,jinfei,numjinfei,dot1,dot2)
...
for i = 1:size(popinitial,1)           
    [validPoints]=checknode_singlepath_close(popinitial(i,:),varDim,buildings,numBuildings,cylinders,numCylinders,spheres,numSpheres,pyramids,numPyramids,jinfei,numjinfei);%对当前解包含的航迹点进行碰撞检测    
    a=[a;validPoints'];
    % c=[c;bj'];
       if any(~validPoints) 
       b=[b;0];
...
for i=1:size(popa,1)
    ...
    yb=auxbpopa(i,:);
    % yc=auxcpopa(i,:);
    [auxa,auxb,auxall,pop,objvs,clustTag,clustName,centroid]=OACES_(auxall,y,yObjv,ypopulation,ya,yb,pop,objvs,clustTag,auxa,auxb,auxc,clustName,centroid,varDim,Kmax);
end
end
...
function [auxa,auxb,auxall,pop,objvs,clustTag,clustName,centroid]=OACES_(auxall,y,yObjv,ypopulation,ya,yb,pop,objvs,clustTag,auxa,auxb,auxc,clustName,centroid,varDim,Kmax)
if isempty(clustName)%如果所有先前分好类的个体为空，即全部被删除
    pop=[pop;y]; objvs=[objvs;yObjv];auxall=[auxall,ypopulation]; 
    auxa=[auxa;ya];auxb=[auxb;yb];
    % auxc=[auxc;yc];
