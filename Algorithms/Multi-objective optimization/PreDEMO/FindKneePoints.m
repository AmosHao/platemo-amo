function [Pbestind, PbestF] = FindKneePoints(Parent,K)
...
Extreme = FindCornerSolutions(PopObj);
...
% 
% T = clusterdata(PopObj,'maxclust',K,'distance','euclidean','linkage','ward');

[C] = Clustering(PopObj, K);

Kneepts  = [];
for i = 1 : K
    pr             = C{i};
    cpr            = Distance(pr);
    [~,rank_kn]    = sort(cpr,'descend');
    kp             = rank_kn(1);  
    nn             = pr(kp);
    Kneepts        = [Kneepts nn];   
end

Pbestind   = Parent(Kneepts).decs;
PbestF     = PopObj(Kneepts,:);
