...
%--------------------------------------------------------------------------
    ...
    if size(PopObj,2) ~= size(optimum,2)
        ...
        % 根据实际目标值范围调整（含惩罚项后最大估算）：
        %   f1（能量+惩罚）：3架×2000kJ能耗 + 3架×1100kJ惩罚 ≈ 9.3×10^6 J，取 1×10^7 留余量
        %   f2（时间+惩罚）：最大时间代价 ~3×10^5 s，取 4×10^5 留余量
        % 必须保证 fmax 大于所有可能的目标值，否则超界解被截断导致 HV 曲线震荡
        fmax = [1e7, 4e5];  % [总能量+惩罚上限(J), 总时间+惩罚上限(s)]
