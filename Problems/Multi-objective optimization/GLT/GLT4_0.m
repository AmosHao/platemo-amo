 ...
 methods
        ...
        if size(PopDec,1)>1
             for i = 1:size(PopDec,1)
                ...
                PopObj(i,2)= (1+g(i,1))*(2-2*PopDec(i,1).^(0.5)*cos(2*PopDec(i,1).^0.5*pi).^2);

             % g         = sum((x(2:pd)-cos(2*pi*x(1)+(2:pd)*pi/pd)).^2);
             % y(1)      = (1+g)*x(1);
             % y(2)      = (1+g)*(2-2*x(1).^(0.5)*cos(2*x(1).^0.5*pi).^2);
             end
        else
             g(1,1) = sum((PopDec(1,2:D)-sin(2*pi*PopDec(1,1)+(2:D)*pi/D)).^2);
             PopObj(1,1)= (1+g(1,1))*PopDec(1,1);
             PopObj(1,2)= (1+g(1,1))*(2-2*PopDec(1,1).^(0.5)*cos(2*PopDec(1,1).^0.5*pi).^2);
        end
        end
