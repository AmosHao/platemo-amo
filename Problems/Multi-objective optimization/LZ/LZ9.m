...
% ss      = x - sin(6*pi*PopDec(i,1) +j*pi/D);
                 g(j,1)=PopDec(i,j)- sin(6*pi*PopDec(i,1) +j*pi/D);
                 
                
                 end
                 PopObj(i,1)= PopDec(i,1) + 2.0/length(io)*sum(g(io,1).^2.0);
                 % y(2)    = 1.0 - x(1).*x(1)  + 2.0/length(ie)*sum(ss(ie).^2);
                 PopObj(i,2)=1.0 - PopDec(i,1).*PopDec(i,1) + 2.0/length(ie)*sum(g(ie,1).^2.0);
             end
        else
            for j=2:D
                
                 g(j,1)=PopDec(1,j)- sin(6*pi*PopDec(1,1) +j*pi/D);
                 
                 end
                 PopObj(1,1)= PopDec(1,1) + 2.0/length(io)*sum(g(io,1).^2.0);
                 PopObj(1,2)=1.0 -  PopDec(1,1).*PopDec(1,1) + 2.0/length(ie)*sum(g(ie,1).^2.0);
