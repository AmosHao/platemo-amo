classdef planner_order_new < PROBLEM
% <multi/many> <real> <large/none> <expensive/none>
 methods
    ...
    if size(PopDec,1)>1
        ...
        % 编码有效性检查
        valid_solution = true;  % 使用标志变量
        
        if length(route) ~= 12
            fprintf('警告：解%d长度不正确: %d\n', j, length(route));
            valid_solution = false;
        end
        
        % 检查0的数量
        if valid_solution
            zero_count = sum(route == 0);
            if zero_count ~= 2
                fprintf('警告：解%d中0的数量不正确: %d\n', j, zero_count);
                valid_solution = false;
            end
        end
        ...
        % 检查客户编号范围
        if valid_solution
            if any(customers < 1) || any(customers > 10)
                fprintf('警告：解%d中客户编号超出范围\n', j);
                valid_solution = false;
            end
        end
        ...
        % 检查重复客户
        if valid_solution
            if length(unique(customers)) ~= 10
                fprintf('警告：解%d中有重复客户\n', j);
                valid_solution = false;
            end
        end
        ...
        % 如果解无效，跳过处理
        if ~valid_solution
            PopObj(j,:) = [inf, inf];  % 设置为无效解
            continue;  % 在for循环中，continue是有效的
        end
        ...
        for k = 1:m
            ...
            for i = 1:npts-1 
                ...
                % 调试输出：打印距离计算信息
                if j <= 3 && k == 1
                    fprintf('解 %d: 从点%d到点%d, 距离=%.2f\n', j, dot1-1, dot2-1, d_hor);
                end
