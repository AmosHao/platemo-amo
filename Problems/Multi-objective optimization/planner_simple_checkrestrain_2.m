classdef planner_simple_checkrestrain_2 < PROBLEM
% <multi/many> <real> <large/none> <expensive/none>
 methods
        ...
        function Setting(obj)
            ...
            %场景设置
            if isempty(obj.dots); obj.dots = [50,50,800,600,600,800]; end
            ...
            % % dot2=[1800,8950,800];
            % dot2=[7000,3000,800];
            D_restrain=100;z_min=800;z_max=1000;
            % n_dots=obj.D/3;
...
radius = 170;
   ...
   [DIS]=checknode_singlepath_close_dis(PopDec(j,:),varDim,buildings,numBuildings,cylinders,numCylinders,spheres,numSpheres,pyramids,numPyramids,jinfei,numjinfei);%对当前解包含的航迹点进行碰撞检测      
   % if any(~validPoints) 
   % b=[b;0];
   % else   
   % b=[b;1];
...
Penalty7=100*Penalty7;
% Penalty1=Penalty1/1000;
% Penalty2=Penalty2/1000;
% Penalty3=Penalty3/1000;
% Penalty6=Penalty6/1000;
% Penalty7=Penalty7/1000;
% 
% Penalty5=Penalty5/1000;
PopObj(j,1)=PopObj(j,1)/1000;
        PopObj(j,1)=PopObj(j,1)+(Penalty1+Penalty2+Penalty3+Penalty4+Penalty5+Penalty6+Penalty7);
        PopObj(j,2)=PopObj(j,2)+(Penalty1+Penalty2+Penalty3+Penalty4+Penalty5+Penalty6+Penalty7);
        PopObj(j,3)=PopObj(j,3)+(Penalty1+Penalty2+Penalty3+Penalty4+Penalty5+Penalty6+Penalty7);
        end
