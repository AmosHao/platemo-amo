classdef planner_travel_maxhjd02 < PROBLEM
% <multi/many> <real> <large/none> <expensive/none>
 methods
        ...
        function Setting(obj)
            ...
            %场景设置
            if isempty(obj.dots); obj.dots = [500,500,800,7000,3000,800]; end
            dots=obj.dots;
            ...
            % % dot2=[1800,8950,800];
            % dot2=[7000,3000,800];
            D_restrain=50;z_min=800;z_max=2400;
            % n_dots=obj.D/3;
            ...
            n_dots=ceil(1.2*distanse_dots/D_restrain);%向上取整
        ...
        D_restrain=50;z_min=800;z_max=2400;
            %长方形
            xLimits = [6500 ,8000;9100 ,10000;2000, 3100;6800, 8100];
            yLimits = [7000 ,8700;6300 ,8600;1200, 2200;3300, 4400];
            %平行四边形
        ...
        if PopObj(j,1)>1.2*distanse_dots
        ...
        % PopObj(j,3)=sum(dis_juxing)+sum(dis_yuan);
         ...
         for i = 1:n
            ...
            % 计算点至长方形的最近距离
            dis_juxing(i)=0;
            for d=1:size(xLimits,1)
            if (currentX >= xLimits(d,1) && currentX <= xLimits(d,2) && currentY >= yLimits(d,1) && currentY <= yLimits(d,2))
                dis_juxing(i)=dis_juxing(i)+100 ;% 点在长方形内部
            % else
            %     dis_juxing(i)=dis_juxing(i)+sqrt((currentX-(xLimits(d,1)+xLimits(d,2))/2).^2+(currentY-(yLimits(d,1)+yLimits(d,2))/2).^2+(currentZ-0).^2);
            end
            end
           % 计算点到平行四边形的最小距离
        ...
        % PopObj(j,3)=1000000000000/(sum(dis_yuan)+sum(dis_juxing)+sum(touying));
if sum(dis_yuan)+sum(dis_juxing)+sum(touying)==0
    PopObj(j,3)=100;
else
        PopObj(j,3)=(sum(dis_yuan)+sum(dis_juxing)+sum(touying));
end
PopObj(j,3)=PopObj(j,3)*100;
Penalty1=10*Penalty1;
Penalty2=100*Penalty2;
