classdef planner_travel_maxhjd_newobj_fangzhen < PROBLEM
% 复制自 planner_travel_maxhjd_newobj：目标3 与禁飞/噪音/人流区 使用 order_data_fangzhen（planner_fangzhen_scene）
% <multi/many> <real> <large/none> <expensive/none>
 methods
        ...
        %场景设置
            ...
            Senv = planner_fangzhen_scene(repoRoot);
            xLimits = Senv.xLimits; yLimits = Senv.yLimits; circleCenter = Senv.circleCenter; radius = Senv.radius;
        distanse_dots=sqrt((dot1(1)-dot2(1)).^2+(dot1(2)-dot2(2)).^2+(dot1(3)-dot2(3)).^2);
        if size(PopDec,1)>1
        for j = 1:N 
        ...
        %计算目标三：禁飞/噪音矩形(到中心距离) + 人流圆，高斯型
        PopObj(j,3) = fangzhen_popobj3_from_env(x, y, nNodes, xLimits, yLimits, circleCenter, radius);
...
function v3 = fangzhen_popobj3_from_env(x, y, nNodes, xLimits, yLimits, circleCenter, radius)
% 矩形区：中心 + 半对角长 rj 作高斯尺度；圆区：crowded 半径
    acc = 0;
    for k = 1:size(xLimits, 1)
        cx = (xLimits(k, 1) + xLimits(k, 2)) / 2;
        cy = (yLimits(k, 1) + yLimits(k, 2)) / 2;
        w  = xLimits(k, 2) - xLimits(k, 1);
        h  = yLimits(k, 2) - yLimits(k, 1);
        rj = 0.5 * sqrt(w^2 + h^2) + 1e-6;
        for i = 1:nNodes-2
            d2 = (x(i+1) - cx)^2 + (y(i+1) - cy)^2;
            acc = acc + exp(-d2 / (rj^2));
        end
    end
    for k = 1:size(circleCenter, 1)
        rr = radius(k) + 1e-9;
        for i = 1:nNodes-2
            d2 = (x(i+1) - circleCenter(k,1))^2 + (y(i+1) - circleCenter(k,2))^2;
            acc = acc + exp(-d2 / (rr^2));
        end
    end
    v3 = acc;
