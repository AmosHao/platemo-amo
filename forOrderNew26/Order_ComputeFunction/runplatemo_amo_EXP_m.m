...
for a = 1:length(algorithm)
    ...
    [summaryTable, hv_end_all, igd_end_all] = ...
        run_ablation_exp_parallel_mat(problem, alg, numRuns, N, maxFE, saveInterval, M, expNameCur, outRootDir);
    fprintf('\n===== %s 汇总（%d 次运行）=====\n', algTag, numRuns);
    disp(summaryTable);
...
function [summaryTable, hv_end_all, igd_end_all] = ...
    run_ablation_exp_parallel_mat(problem, algorithm, numRuns, N, maxFE, saveInterval, M, expName, outRootDir)
