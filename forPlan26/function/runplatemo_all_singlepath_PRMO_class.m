...
function [validtrait_end,validtrait,clustName,clustTag,clustName_end,clustTag_end,Dec,Obj,Dec_end,Obj_end,hv,hv_end,pd,pd_end]=runplatemo_all_singlepath_PRMO_class(problem,algorithm,N,maxFE,s,M,dots)
...
for j = 1:size(result, 1)%1-20
for i = 1:numel(result{j, end})%1-100
    % popgen = zeros(numel(result{j, end}), num_dec_columns);
    popgen(i,:) =result{j,2}(1,i).dec; %jde100gebianli   
end
...
% 
% % 创建唯一的文件名
% filename = sprintf('0103_dot1_%s_dot2_%s.xlsx', dot1_str, dot2_str);
% 
% % 将 obj 矩阵保存到 'Sheet1'
